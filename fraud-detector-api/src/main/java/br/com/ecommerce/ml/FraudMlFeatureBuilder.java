...
public final class FraudMlFeatureBuilder {
    ...
    private static final List<String> AMOUNT_BUCKETS = List.of(
            "low",
            "medium",
            "high",
            "very_high"
    ...
    private static final List<String> ITEMS_BUCKETS = List.of(
            "single",
            "few",
            "many"
    );
    ...
    public static ArrayExample<Label> buildExample(
        ...
        double itemsQuantityValue = toDouble(itemsQuantity);
        double avgItemPriceValue = toDouble(avgItemPrice);
        double maxItemPriceValue = toDouble(maxItemPrice);
        double uniqueProductsValue = toDouble(uniqueProducts);
        ...
        double routeDistanceKm = haversineKm(origin, destination);
        String distanceBucket = distanceBucket(routeDistanceKm);
        ...
        features.add(new Feature("route_distance_km", routeDistanceKm));
        ...
        for (String bucket : AMOUNT_BUCKETS) {
            features.add(new Feature(
                    "amount_" + bucket,
                    bucket.equals(amountBucket) ? 1.0 : 0.0
            ));
        }

        for (String bucket : ITEMS_BUCKETS) {
            features.add(new Feature(
                    "items_" + bucket,
                    bucket.equals(itemsBucket) ? 1.0 : 0.0
            ));
        }
